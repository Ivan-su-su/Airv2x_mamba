...
class BaseBEVResBackbone(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        for i in range(len(self.blocks)): # 4
            x = self.blocks[i](x) # torch.Size([2, 128, 360, 360]) -> torch.Size([2, 128, 180, 180]) -> torch.Size([2, 256, 90, 90]) -> torch.Size([2, 512, 45, 45]) 

            stride = int(spatial_features.shape[2] / x.shape[2]) # 1 -> 2 -> 4 -> 8
            ret_dict['spatial_features_%dx' % stride] = x
            if len(self.deblocks) > 0:
                ups.append(self.deblocks[i](x)) # torch.Size([2, 128, 180, 180])
            else:
                ups.append(x)

        if len(ups) > 1:
            x = torch.cat(ups, dim=1) # torch.Size([2, 512, 180, 180])
        elif len(ups) == 1:
            x = ups[0]
